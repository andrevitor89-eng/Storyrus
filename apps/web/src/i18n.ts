...
export function validityHandlers(lang: Lang) {
  const t = SHARED[lang];
  return {
    onInvalid(e: { currentTarget: HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement }) {
      const el = e.currentTarget;
      if (!el.value.trim() || el.validity.valueMissing) el.setCustomValidity(t.required);
      else if ("typeMismatch" in el.validity && el.validity.typeMismatch) el.setCustomValidity(t.email_invalid);
