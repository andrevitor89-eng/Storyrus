...
def test_seed_copies_amazonia_avatar_once(tmp_path, monkeypatch):
    amazonia = tmp_path / "amazonia-matteo"
    amazonia.mkdir()
    src = amazonia / "character.png"
    src.write_bytes(_blob(b"char-amazonia"))
    dest_dir = tmp_path / "cores-matteo"
    dest = dest_dir / "character.png"

    monkeypatch.setattr(cores, "APPROVED_CHAR", src)
    monkeypatch.setattr(cores.SPEC, "out_dir", dest_dir)

    cores.seed_approved_character()
    assert dest.read_bytes() == src.read_bytes()

    dest.write_bytes(_blob(b"char-cores"))
    cores.seed_approved_character()
    assert dest.read_bytes() == _blob(b"char-cores")
